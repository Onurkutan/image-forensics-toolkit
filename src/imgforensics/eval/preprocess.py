...
class AugmentationConfig(BaseModel):
    ...
    than one can fire on the same image).
    """
...
def augment(rgb: Image.Image, config: AugmentationConfig, rng: np.random.Generator) -> Image.Image:
    """Apply ``config``'s enabled augmentations to ``rgb``, each with probability ``config.p``.
    ...
    application: downscale/upscale, blur, noise, JPEG, WEBP, cutout.
    ...
    if config.downscale_upscale is not None and rng.random() < config.p:
        result = _apply_downscale_upscale(result, config.downscale_upscale, rng)
    if config.gaussian_blur_sigma is not None and rng.random() < config.p:
        result = _apply_blur(result, config.gaussian_blur_sigma, rng)
    if config.noise_sigma is not None and rng.random() < config.p:
        result = _apply_noise(result, config.noise_sigma, rng)
    if config.jpeg_quality is not None and rng.random() < config.p:
        result = _apply_jpeg(result, config.jpeg_quality, rng)
    if config.webp_quality is not None and rng.random() < config.p:
        result = _apply_webp(result, config.webp_quality, rng)
    if config.cutout is not None and rng.random() < config.p:
